...
static void write_outcome(const char *wrapper, const char *domain, const char *symbol, int code, int passed)
{
    ...
    {
        written = fprintf(outcome_stream, "P101WRAPPER\t1\tFAULT\t%s\tlib_dynamic_linking\t%s\t%s\t%s\t%d\t%s\n", P101_TEST_PLATFORM, wrapper, domain, symbol, code, passed ? "PASS" : "FAIL");
...
/* P101_TEST_CASE(p101_dlclose) */
static void test_p101_dlclose(struct p101_env *env, struct p101_error *err)
{
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        int result = p101_dlclose(env, err, argument_2);
        (void)result;
        ...
        write_outcome("p101_dlclose", "system", error_names[index], state.code, failures == failures_before);
    ...
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            native_argument_2 = dlopen(NULL, RTLD_LAZY);
            if(native_argument_2 == NULL)
            {
                native_child_status = 77;
                goto native_child_done_;
            }
            int native_result = p101_dlclose(native_env, native_err, native_argument_2);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                ...
                {
                    fprintf(stderr, "native smoke produced an undeclared platform failure: p101_dlclose: %s\n", p101_error_get_message(native_err));
                    native_passed = false;
                }
            ...
            if(native_result != 0)
            {
                P101_NATIVE_CLEANUP_STATUS(dlclose(native_argument_2));
            }
...
/* P101_TEST_CASE(p101_dlopen) */
static void test_p101_dlopen(struct p101_env *env, struct p101_error *err)
{
#ifdef __linux__
    static const int         errors[]      = {EINVAL};
    static const char *const error_names[] = {"EINVAL"};
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        errno                 = P101_TEST_ERRNO_SENTINEL;
        p101_env_set_fault_injector(env, fail_next_call, &state);
        void *result = p101_dlopen(env, err, NULL, 0);
        (void)result;
        EXPECT(state.checks == 1);
        EXPECT(p101_error_is_error(err, P101_ERROR_SYSTEM, state.code));
        EXPECT(errno == P101_TEST_ERRNO_SENTINEL);
        EXPECT(result == (NULL));
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_dlopen", "system", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            void *native_result = p101_dlopen(native_env, native_err, NULL, RTLD_LAZY);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                bool native_error_declared = false;
                ...
                if(!native_error_declared)
                {
                    fprintf(stderr, "native smoke produced an undeclared platform failure: p101_dlopen: %s\n", p101_error_get_message(native_err));
                    native_passed = false;
                }
                p101_error_reset(native_err);
            }
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_dlopen: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status) && WEXITSTATUS(native_status) == 77)
            {
                fprintf(stderr, "native smoke fixture unavailable: p101_dlopen\n");
            }
            else if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_dlopen: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
/* P101_TEST_CASE(p101_dlsym) */
static void test_p101_dlsym(struct p101_env *env, struct p101_error *err)
{
    unsigned char argument_2[64];
    unsigned char argument_2_before[sizeof(argument_2)];
    memset(argument_2, 0xA5, sizeof(argument_2));
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        errno                 = P101_TEST_ERRNO_SENTINEL;
        p101_env_set_fault_injector(env, fail_next_call, &state);
        void *result = p101_dlsym(env, err, argument_2, NULL);
        (void)result;
        EXPECT(state.checks == 1);
        EXPECT(p101_error_is_error(err, P101_ERROR_SYSTEM, state.code));
        EXPECT(errno == P101_TEST_ERRNO_SENTINEL);
        ...
        EXPECT(memcmp(argument_2, argument_2_before, sizeof(argument_2)) == 0);
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_dlsym", "system", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            void *native_result = p101_dlsym(native_env, native_err, RTLD_DEFAULT, "malloc");
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                bool native_error_declared = false;
                ...
                if(!native_error_declared)
                {
                    fprintf(stderr, "native smoke produced an undeclared platform failure: p101_dlsym: %s\n", p101_error_get_message(native_err));
                    native_passed = false;
                }
                p101_error_reset(native_err);
            }
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_dlsym: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status) && WEXITSTATUS(native_status) == 77)
            {
                fprintf(stderr, "native smoke fixture unavailable: p101_dlsym\n");
            }
            else if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_dlsym: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
int main(void)
{
    ...
    else
    {
        ...
        {
            test_p101_dlclose(env, err);
        }
        if(!native_child_process)
        {
            test_p101_dlopen(env, err);
        }
        if(!native_child_process)
        {
            test_p101_dlsym(env, err);
        }
    }
